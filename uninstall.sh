...
Options:
  ...
  --remove-config   Remove config directory at $CONFIG_DIR
  --remove-db       Remove database file at $DB_PATH
EOF
}
...
if [[ "$REMOVE_CONFIG" == "1" && -d "$CONFIG_DIR" ]]; then
  ...
  as_user rm -rf "$CONFIG_DIR"
fi

if [[ "$REMOVE_DB" == "1" && -f "$DB_PATH" ]]; then
  echo "Removing database: $DB_PATH"
  as_user rm -f "$DB_PATH"
fi
