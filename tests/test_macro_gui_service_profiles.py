...
def test_on_printer_mode_keeps_explicit_moonraker_url(tmp_path: Path) -> None:
    ...
    on_printer_service = MacroGuiService(
        db_path=tmp_path / "vault.db",
        config_dir=tmp_path / "config",
        version_history_size=5,
        runtime_mode="on_printer",
        moonraker_base_url="http://127.0.0.1:7125",
    )

    url = on_printer_service._moonraker_url("/printer/restart")
    assert url.startswith("http://127.0.0.1:7125")
...
def test_off_printer_index_syncs_remote_before_index(tmp_path: Path) -> None:
    ...
    with (
        ...
        patch(
            "klipper_macro_gui_service.run_indexing",
            return_value={"cfg_files_scanned": 2, "macros_inserted": 1, "macros_unchanged": 0},
        ),
    ):
